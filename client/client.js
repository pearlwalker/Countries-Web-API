...
const submitGetEvery = async (form, method, options, url) => {
    ...
    for (const country of countries) {
        resText += `<h3 class="js_h3">${country.name}</h3>`;
        resText += `<ul class="js_ul">`;
        checklistData
            ...
            .forEach((item) => {
                switch (true) {
                    ...
                    case (item.attr === "finance"):
                        ...
                        resText += `currency: ${country.finance["currency_name"]} (${country.finance["currency_symbol"]}/${country.finance.currency})`;
                        resText += `</li>`;
                        break;
